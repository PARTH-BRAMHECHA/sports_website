...
const ManageSchedule = () => {
  ...
  const fetchSchedules = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:4000/api/admin/schedules', {
        headers: { 
  ...
  const fetchEvents = async () => {
    try {
      ...
      } else if (response.data && typeof response.data === 'object' && response.data.events) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (selectedSchedule) {
        await axios.put(
          `http://localhost:4000/api/admin/schedules/${selectedSchedule._id}`,
          formData,
          {
            headers: { 
              'Authorization': `Bearer ${user?.token}`
        ...
        await axios.post('http://localhost:4000/api/admin/schedules', formData, {
          headers: { 
            'Authorization': `Bearer ${user?.token}`
          }
        });
      ...
      setError(error.response?.data?.error || error.message || 'An error occurred while saving the schedule');
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this schedule?')) {
      try {
        await axios.delete(`http://localhost:4000/api/admin/schedules/${id}`, {
          headers: { 
            'Authorization': `Bearer ${user?.token}`
          }
        });
  ...
  const handleInitializeData = async () => {
    try {
      ...
      await axios.post('http://localhost:4000/api/admin/schedules', defaultSchedule, {
        headers: { 
          'Authorization': `Bearer ${user?.token}`
        }
      });
    ...
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      ...
      {!Array.isArray(schedules) ? (
        <Alert severity="error">
          There was an issue loading the schedules data. Please try refreshing the page.
        </Alert>
      ) : schedules.length > 0 ? (
            ...
            <TableBody>
              {schedules.map((schedule) => (
                <TableRow key={schedule._id || Math.random().toString()}>
                  ...
                  <TableCell>{schedule.eventId ? getEventNameById(schedule.eventId) : 'General'}</TableCell>
                  <TableCell>{Array.isArray(schedule.days) ? schedule.days.length : 0}</TableCell>
                  <TableCell>
      ...
      <Dialog open={open} onClose={handleClose} maxWidth="lg" fullWidth>
        <DialogTitle>
          {selectedSchedule ? 'Edit Event Schedule' : 'Create New Event Schedule'}
        </DialogTitle>
          ...
          <DialogContent>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} md={6}>
                <FormControl fullWidth margin="normal">
                  <InputLabel id="event-select-label">Associated Event (Optional)</InputLabel>
                  <Select
            ...
            <Box sx={{ mt: 3, mb: 1 }}>
              ...
              {!formData.days || formData.days.length === 0 ? (
                <Paper sx={{ p: 2, mt: 1, textAlign: 'center', bgcolor: 'action.hover' }}>
                  ...
                  >
                    <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', pr: 2 }}>
                        ...
                        />
                        <Typography variant="body2" color="text.secondary">
                          {day.events && day.events.length} {day.events && day.events.length === 1 ? 'event' : 'events'}
                        </Typography>
                      </Box>
                    </AccordionSummary>
                    <AccordionDetails>
                              ...
                              <ListItem 
                                secondaryAction={
                                  <Box>
                                    <IconButton 
                                      edge="end" 
                                      onClick={() => handleOpenEventDialog(dayIndex, eventIndex)}
                                    >
                                ...
                                <ListItemText
                                  primary={
                                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                                      <Typography variant="body2" component="span" sx={{ fontWeight: 'bold', minWidth: '80px' }}>
                                        {event.time}
                                      </Typography>
